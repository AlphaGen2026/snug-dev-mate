    ...
    <header className="fixed inset-x-0 top-0 z-50 border-b border-border/60 bg-background/90 backdrop-blur-xl">
      <div className="mx-auto flex h-18 max-w-7xl items-center justify-between px-5 lg:px-8">
        ...
        <nav aria-label="Main navigation" className="hidden items-center gap-8 md:flex">
          {t.nav.map((item, i) => <button key={item} onClick={() => document.getElementById(["capabilities", "security", "specialists"][i])?.scrollIntoView({behavior:"smooth"})} className="text-sm font-medium text-muted-foreground transition-colors hover:text-foreground">{item}</button>)}
        </nav>
